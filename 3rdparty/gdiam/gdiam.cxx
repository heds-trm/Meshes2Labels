                         ...
                         const point2d  & c )
{
    ...
    area = a.x * b.y - a.y * b.x +
		...
		if(area < 1e-12 && area > -1e-12 ) area = (ldouble)0.0;
    //printf( "area: %g\n", area );
...
void  convex_hull( vec_point_2d  & in, vec_point_2d  & out )
{
    ...
    printf( "comp.base: (%g, %g)\n",
            ...
            comp.base.y );
    */
    for  ( int  iind = 0; iind < (int)in.size(); iind++ ) {
        assert( in[ iind ] != NULL );
    }
...
gdiam_bbox   gdiam_mvbb_optimize( gdiam_point  * start, int  size,
                                  gdiam_bbox  bb_out, int  times, bool& error  )
{
    gdiam_bbox  bb_tmp;
...
gdiam_bbox   gdiam_approx_mvbb( gdiam_point  * start, int  size,
                                gdiam_real  eps, bool& error ) 
{
    gdiam_bbox  bb, bb2;

    bb = gdiam_approx_const_mvbb( start, size, 0.0, NULL );
		error = true;
    bb2 = gdiam_mvbb_optimize( start, size,  bb, 10, error );
                            ...
                            int  z_coef )
{
    ...
    bb_new = pps.get_bbox();
		// js
		bool error = true;
    bb_new = gdiam_mvbb_optimize( start, size, bb_new, 10, error );
                                     ...
                                     int  grid_size ) 
{
    ...
    if  ( bb.volume() == 0 ) {
		...
		// js
		bool error = true;
    bb_out = bb = gdiam_mvbb_optimize( start, size, bb_out, 10, error );
    ...
    for  ( int  x_coef = -grid_size; x_coef <= grid_size; x_coef++ )
		...
		// js
		error = true;
    bb_out = gdiam_mvbb_optimize( start, size, bb_out, 10, error );
